...
class ModernBertConfig(PretrainedConfig):
    ...
    def __init__(
        ...
        rope_theta=13500.0, # added by Hannahvandenbos
        ...
        **kwargs,
        ...
        self.rope_theta = rope_theta # added by Hannahvandenbos
...
class ModernBertRotaryEmbedding(GemmaRotaryEmbedding):
    def __init__(self, config: ModernBertConfig, dim: int, base: float, device: Optional[torch.device] = None):
        super().__init__(config=config, device=device) # self removed by Hannahvandenbos
...
def eager_attention_forward(
    ...
    zero_value_index: Optional[int] = None,  # added by Hannahvandenbos
    **_kwargs,
    ...
    if zero_value_index is not None: # zeroing value vectors corresponding to the given token index
        value[:, :, zero_value_index] = 0.0 # added by Hannahvandenbos
...
def sdpa_attention_forward(
    ...
    bs: int,
    dim: int,
    zero_value_index: Optional[int] = None,  # added by Hannahvandenbos    **_kwargs,
) -> Tuple[torch.Tensor]:
    # qkv: [batch_size, seqlen, 3, nheads, headdim]
    cos, sin = module.rotary_emb(qkv, position_ids=position_ids)
    query, key, value = qkv.transpose(3, 1).unbind(dim=2)
...
class ModernBertNormOutput(nn.Module):
    ...
    def forward(
        ...
        with torch.no_grad():
            # Transform value vectors (V * W^o)
            Wo = dense_weight.view(self.all_head_size, self.num_attention_heads, self.attention_head_size)
            transformed_layer = torch.einsum('bhsv,dhv->bhsd', value_layer, Wo)

            # Apply attention weights: α * f(x)
            ...
            residual = torch.einsum('sk,bsd->bskd', torch.eye(seq_len, device=hidden_states.device), hidden_states)
            ...
            mean = residual_weighted_layer.mean(-1, keepdim=True)
            var = (residual_weighted_layer - mean).pow(2).mean(-1, keepdim=True)
            normalized = (residual_weighted_layer - mean) / torch.sqrt(var + layernorm_eps)
            post_ln_layer = normalized * layernorm_weight  # broadcast multiply

            if not globenc_only:
                post_ln_norm = torch.norm(post_ln_layer, dim=-1)

                # Mixing ratios
                def mix_ratio(tensor):
                    preserve = torch.diagonal(tensor, dim1=1, dim2=2).permute(0, 2, 1)
                    mix = tensor.sum(dim=2) - preserve
                    return torch.norm(mix, dim=-1) / (torch.norm(mix, dim=-1) + torch.norm(preserve, dim=-1))

                attn_n_mixing_ratio = mix_ratio(summed_weighted_layer)
                attnres_n_mixing_ratio = mix_ratio(residual_weighted_layer)
                attnresln_n_mixing_ratio = mix_ratio(post_ln_layer)
                ...
                return (
                    ...
                    post_ln_layer,
                    attn_n_mixing_ratio,
                    attnres_n_mixing_ratio,
                    attnresln_n_mixing_ratio,
                )
            else:
...
class ModernBertAttention(nn.Module):
    ...
    def __init__(self, config: ModernBertConfig, layer_id: Optional[int] = None):
        ...
        self.norm = ModernBertNormOutput(config)  # added by Goro Kobayashi (Hannahvandenbos)
    ...
    def forward(
        ...
        output_attentions: Optional[bool] = False,
        zero_value_index=None, # added by Hannahvandenbos
        output_norms: Optional[bool] = False,  # added by Fayyaz / Modarressi (Hannahvandenbos)
        output_globenc: Optional[bool] = False,  # added by Fayyaz / Modarressi (Hannahvandenbos)
        **kwargs,
    ) -> torch.Tensor:
        ...
        attn_outputs = MODERNBERT_ATTENTION_FUNCTION[self.config._attn_implementation](
            ...
            zero_value_index=zero_value_index, # added by Hannahvandenbos
            **kwargs,
        )
        hidden_states = attn_outputs[0]
        hidden_states = self.out_drop(self.Wo(hidden_states))

        # attention_output = (attn_outputs[0], hidden_states, output_norms or output_globenc,)  # added by Goro Kobayashi (Edited by Fayyaz / Modarressi) (Hannahvandenbos)
        # # added by Fayyaz / Modarressi (Hannahvandenbos)
        # # -------------------------------
        if output_norms or output_globenc:
            attention_probs, value_layer = attn_outputs

            norm_outputs = self.norm(
                hidden_states=hidden_states,
                attention_probs=attention_probs,
                value_layer=value_layer,
                dense_weight=self.Wo.weight,
                layernorm_weight=self.config.layernorm_weight, 
                pre_ln_states=None,
                globenc_only=not output_norms
            )

            return (hidden_states, attention_probs) + norm_outputs
...
class ModernBertEncoderLayer(nn.Module):
    ...
    def forward(
        ...
        output_attentions: Optional[bool] = False,
        zero_value_index=None, # added by Hannahvandenbos
        output_norms: Optional[bool] = False,  # added by Goro Kobayashi (Hannahvandenbos)
        output_globenc: Optional[bool] = False,  # added by Fayyaz / Modarressi (Hannahvandenbos)
    ) -> torch.Tensor:
        attn_outputs = self.attn(
            ...
            output_attentions=output_attentions,
            zero_value_index=zero_value_index, # added by Hannahvandenbos
            output_norms=output_norms,  # added by Goro Kobayashi (Hannahvandenbos)
            output_globenc=output_globenc, # added by Fayyaz / Modarressi (Hannahvandenbos)
        )
        ...
        # # added by Fayyaz / Modarressi (Hannahvandenbos)
        # # -------------------------------
        pre_ln2_states = hidden_states  # Save pre-MLP residual
        hidden_states = hidden_states + mlp_output  # Final output

        if output_norms or output_globenc:
            # LayerNorm stats
            post_ln_mean = hidden_states.mean(-1, keepdim=True)
            pre_ln_mean = pre_ln2_states.mean(-1, keepdim=True)
            pre_ln_var = (pre_ln2_states - pre_ln_mean).pow(2).mean(-1, keepdim=True).unsqueeze(2)

            # Normalize the hidden states
            normalized = (hidden_states - post_ln_mean) / torch.sqrt(pre_ln_var + self.mlp_norm.eps)

            # Apply learned LayerNorm weights
            normed_output = normalized * self.mlp_norm.weight.view(1, 1, -1)

            if output_globenc:
                post_ln2_norm = torch.norm(normed_output, dim=-1)  # shape: (batch_size, seq_len)

            if output_norms:
                # Diagonal = self-preserving path; rest = mixing
                diag = torch.diagonal(normed_output, dim1=1, dim2=2).permute(0, 2, 1)
                mixing = torch.sum(normed_output, dim=2) - diag
                preserving_norm = torch.norm(diag, dim=-1)
                mixing_norm = torch.norm(mixing, dim=-1)
                mixing_ratio = mixing_norm / (mixing_norm + preserving_norm + 1e-6)

                if output_globenc:
                    return (hidden_states, post_ln2_norm, mixing_ratio) + attn_outputs[1:]
                else:
                    return (hidden_states, mixing_ratio) + attn_outputs[1:]
            else:
                return (hidden_states, post_ln2_norm) + attn_outputs[1:]
...
class ModernBertModel(ModernBertPreTrainedModel):
    ...
    def forward(
        ...
        output_norms: Optional[bool] = None,  # added by Goro Kobayashi (Hannahvandenbos)
        output_globenc: Optional[bool] = None,  # added by Fayyaz / Modarressi (Hannahvandenbos)
        ...
        all_norms = None # added by Goro Kobayashi (Hannahvandenbos)
        globenc_attributions = None # added by Fayyaz / Modarressi (Hannahvandenbos)
        ...
        # added by Goro Kobayashi (Hannahvandenbos)
        # -------------------------
        if output_norms:
        ...
        if not return_dict:
            ...
            return tuple(v for v in [hidden_states, all_hidden_states, all_self_attentions, globenc_attributions, all_norms] if v is not None)  # Added by Fayyaz / Modarressi (Hannahvandenbos)
