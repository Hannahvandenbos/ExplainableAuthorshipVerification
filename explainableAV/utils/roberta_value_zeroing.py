...
import inspect
from transformers.utils import logging, get_torch_version
...
class RobertaSelfAttention(nn.Module):
    ...
    def forward(
        ...
        #     batch_size, _, seq_len, head_dim = value_layer.size()
        #     for i, idx in enumerate(zero_value_index):
        #         value_layer[i, :, idx] = 0
        
        if zero_value_index is not None: # zeroing value vectors corresponding to the given token index
            value_layer[:, :, zero_value_index] = torch.zeros(value_layer[:, :, zero_value_index].size()) # added by Hannahvandenbos
    ...
    def forward(
        ...
        if self.position_embedding_type != "absolute" or output_attentions or head_mask is not None:
            ...
            return super().forward(
                ...
                past_key_value,
                zero_value_index, # added by Hannahvandenbos
                output_attentions,
            )
        ...
        # if zero_value_index is not None and zero_value_index is not False:
        #     batch_size, _, seq_len, head_dim = value_layer.size()
        #     for i, idx in enumerate(zero_value_index):
        #         value_layer[i, :, idx] = 0
