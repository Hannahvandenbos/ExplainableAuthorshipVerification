...
class RobertaSdpaSelfAttention(RobertaSelfAttention):
    ...
    def forward(
        ...
        past_key_value: Optional[Tuple[Tuple[torch.FloatTensor]]] = None,
        output_attentions: Optional[bool] = False,
        output_norms: Optional[bool] = False,  # added by Hannahvandenbos
        output_globenc: Optional[bool] = False,  # added by Hannahvandenbos
    ) -> Tuple[torch.Tensor]:
        if self.position_embedding_type != "absolute" or output_attentions or head_mask is not None:
            ...
            return super().forward(
                ...
                past_key_value,
                output_attentions,
                output_norms,  # added by Hannahvandenbos
                output_globenc,  # added by Hannahvandenbos
            )
...
class RobertaSelfOutput(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor, input_tensor: torch.Tensor, output_norms=False):  # added by Fayyaz / Modarressi (Hannahvandenbos)
        hidden_states = self.dense(hidden_states)
        hidden_states = self.dropout(hidden_states)
        # hidden_states = self.LayerNorm(hidden_states + input_tensor)
        pre_ln_states = hidden_states + input_tensor  # added by Fayyaz / Modarressi (Hannahvandenbos)
        post_ln_states = self.LayerNorm(pre_ln_states)  # added by Fayyaz / Modarressi (Hannahvandenbos)
        # added by Fayyaz / Modarressi (Hannahvandenbos)
        # -------------------------------
        if output_norms:
...
class RobertaNormOutput(nn.Module):  # This class was added by Goro Kobayashi (Hannahvandenbos)
    def __init__(self, config):
        super().__init__()
        self.num_attention_heads = config.num_attention_heads
        self.attention_head_size = int(config.hidden_size / config.num_attention_heads)
...
class RobertaAttention(nn.Module):
    def __init__(self, config, position_embedding_type=None):
        ...
        self.norm = RobertaNormOutput(config)  # added by Goro Kobayashi (Hannahvandenbos)
    ...
    def forward(
        ...
        past_key_value: Optional[Tuple[Tuple[torch.FloatTensor]]] = None,
        output_attentions: Optional[bool] = False,
        output_norms=False,  # added by Goro Kobayashi (Hannahvandenbos)
        output_globenc=False,  # added by Fayyaz / Modarressi (Hannahvandenbos)
    ) -> Tuple[torch.Tensor]:
        ...
        attention_output = self.output(self_outputs[0], hidden_states, output_norms=output_norms or output_globenc,)  # added by Goro Kobayashi (Edited by Fayyaz / Modarressi) (Hannahvandenbos)
        # added by Fayyaz / Modarressi (Hannahvandenbos)
        # -------------------------------
        if output_norms or output_globenc:
...
class RobertaLayer(nn.Module):
    ...
    def forward(
        ...
        )  # changed by Goro Kobayashi (Hannahvandenbos) 
        ...
        for i, layer_module in enumerate(self.layer):
            ...
            if self.gradient_checkpointing and self.training:
                ...
                layer_outputs = layer_module(
                    ...
                    past_key_value,
                    output_attentions,
                    output_norms,  # added by Goro Kobayashi (Hannahvandenbos)
                    output_globenc # added by Fayyaz / Modarressi (Hannahvandenbos)
                )
        ...
        if not return_dict:
            return tuple(
                ...
                for v in [
                    ...
                    globenc_attributions,  # Added by Fayyaz / Modarressi (Hannahvandenbos)
                    all_norms,             # Added by Fayyaz / Modarressi (Hannahvandenbos)
                ]
                if v is not None
            )
